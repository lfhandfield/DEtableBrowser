...
grid_arrange_shared_legend <- function(plots, ncol = length(plots), nrow = 1, position = c("bottom", "right"), do.share.legend=T,do.newpage=T, main.title=c()) {
  ...
  if (do.share.legend){
    ...
    gl <- c(gl, ncol = ncol, nrow = nrow)
    combined <- arrangeGrob(do.call(arrangeGrob, gl), top= main.title)
  }
...
makeOverlay <- function(overdata, genemat, dropout, gene, compset, titles, gridsize){
  ...
  for(flist in 1:length(compset)){
    ...
    #  logjs(sum(flt))
    #  logjs(which(flt))
    p <- ggplot(gdata, aes(x=X,y=Y,fill=Log2FC, alpha=A, shape=S)) + geom_point();
...
plotDataGrid <- function(data, wdata= c(), xdata = c(), ydata =c(), transform=c(), colcolors=c() , override.colnames=c(), plot.attribs=c(),do.zero.center=T, bgcolor = "#BBBBBB", do.cluster = c(T,T) ){
  ...
  if (is.null(override.colnames)) p <- p + scale_x_discrete(limits= (1:dd[2])-0.5, labels= colnames(data$data) )# + xlab(NULL)
    ...
    p <- p + scale_x_continuous(name = override.colnames$axenames[1], limits= c(0,dd[2]), breaks= (1:dd[2])-0.5, labels= override.colnames$bot, sec.axis=dup_axis(labels= override.colnames$top, breaks= (1:dd[2])-0.5, name = override.colnames$axenames[2]) )# + xlab(NULL)
  }
