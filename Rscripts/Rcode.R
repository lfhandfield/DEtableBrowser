...
printlist <- function(stringlist, append.comma=F, add.quotes=F, subdottodash=F){
	if (subdottodash) stringlist <- gsub("\\.","-", stringlist)
	if (append.comma){
...
makeFancyFilterWilcox <- function(base, curlist, basenames = c("PSENNeuNP", "CtrlPSENNeuNP"), curname="", dropoutthreshold=0.1,selected.colnames=c(),gene.blacklist=c()){
	...
	filter[rownames(filter) %in% gene.blacklist,] <- F
return(filter)}
mergeColSparseMatrices <- function(matrixlist){
    dacoln <- colnames(matrixlist[[1]])
    darown <- rownames(matrixlist[[1]])
    nbdata <- length(matrixlist[[1]]@x)
    for(i in 2:length(matrixlist)){
        if (dim(matrixlist[[i]])[1] != length(darown)) {
            print("warning! incorrect number of rows, increasing")
            darown <- rownames(matrixlist[[i]])
        }
        dacoln <- c(dacoln, colnames(matrixlist[[i]]))
        nbdata <- nbdata + length(matrixlist[[i]]@x)
    }
    fout <- Matrix(0,length(darown), length(dacoln),sparse = T)
    rownames(fout) <- darown
    colnames(fout) <- dacoln
    fout@x = fout@x = rep(0,nbdata)
    fout@i = fout@i = rep(0:0,nbdata) # the way to get integers... do not ask why
    nbdata = 0:0;
    for(i in 1:length(matrixlist)){
	fout@x[(nbdata+1):(nbdata+length(matrixlist[[i]]@x))] = matrixlist[[i]]@x
        fout@i[(nbdata+1):(nbdata+length(matrixlist[[i]]@i))] = matrixlist[[i]]@i        
        fout@p[i] = nbdata
        nbdata = nbdata + length(matrixlist[[i]]@x)
    }
    fout@p[length(matrixlist)+1] = nbdata
return(fout);}
...
createEqualSamples2 <- function(sce, meta.celltype, flt){
	newannot <- rep(F, nrow(colData(sce)))

	count = table(colData(sce)[[meta.celltype]][flt])
	count = count[count != 0]
	cat = min(count)
	print(paste("subsample down to ", cat, "cell per celltype"));
	for( j in levels(colData(sce)[[meta.celltype]])){
		cflt = (colData(sce)[[meta.celltype]] == j)&(flt)
		print(sum(cflt) - cat)
		if (sum(cflt) < cat) next
		newannot[cflt] = sample(c(rep(T,cat),rep(F,sum(cflt) - cat)))
		#for(i in levels(colData(sce)[[meta.sample]])){
		#	flt <- (colData(sce)[[meta.sample]] == i)&(colData(sce)[[meta.celltype]] == j)&(cell.flt)
		#	ord <- order(colData(sce)[[ordering]][flt])
		#	tag <- paste(i, rep(1:1, (sum(flt)/nbparts)+1)[1:sum(flt)], sep="_subsmp") 
		#	newannot[flt][ord] <- tag
		#}
	}
return(newannot);}
...
#	if (length(cell.listNeg) == ncol(sro@data)) cell.listNeg <- colnames(sro@data)[cell.listNeg]
	...
	if (is.null(use.meta.as.replicate)){
		...
		mlvl <- getSafeLevels(metadata[[use.meta.as.replicate]])
		#print("nb")
		#print(table(metadata[[use.meta.as.replicate]][cell.listPos]))
		#print("gffg")
		#print(table(metadata[[use.meta.as.replicate]][cell.listNeg]))
		...
		if (do.permutation.test == "cell"){
			...
			for(i in 1:length(mlvl)){
				flt <- (metadata[[use.meta.as.replicate]] == mlvl[i])
		...
		if (!is.null(bypass.posclass)){
			...
			dataP <- data.frame(row.names = 1:sum(gene.use))
			for(i in 1:length(mlvl)){
				flt <- ( metadata[[use.meta.as.replicate]] == mlvl[i]) & cell.listPos
				#flt2 <- ( metadata[[use.meta.as.replicate]] == mlvl[i]) & cell.listNeg 
				...
				if (sum(flt) >= min.nbcell.threshold) dataP[[paste(mlvl[i],"+",sep="")]] <- Matrix::rowSums(SeuratRawData(sro)[gene.use,flt,drop=F])
			}
		}
		if (!is.null(bypass.negclass)) {
			...
			dataN <- data.frame(row.names = 1:sum(gene.use))
			for(i in 1:length(mlvl)){
				flt <- ( metadata[[use.meta.as.replicate]] == mlvl[i]) & cell.listNeg 
				#flt2 <- ( metadata[[use.meta.as.replicate]] == mlvl[i]) & cell.listPos 
				...
				if (sum(flt) >= min.nbcell.threshold) dataN[[paste(mlvl[i],"-",sep="")]] <- Matrix::rowSums(SeuratRawData(sro)[gene.use,flt,drop=F])
...
SeuratRawData <- function(input, getAttrib=c()){	
	if (is.null(getAttrib)){
		...
		else if (class(input) == "SingleCellExperiment") return(assays(input)[[1]])
		...
		else if (class(input) == "SingleCellExperiment") return(rownames(input))
		else stop("Unknown class for input")
		...
		else if (class(input) == "SingleCellExperiment") return(colnames(input))
		else stop("Unknown class for input")
		...
		else if (class(input) == "SingleCellExperiment") return(dim(assays(input)[[1]]))
		else stop("Unknown class for input")
	...
	}else if (grepl("[Mm]", getAttrib)){
		if (class(input) == "Seurat") return(input@meta.data)
		else if (class(input) == "seurat") return(input@meta.data)
		else if (class(input) == "SingleCellExperiment") return(colData(input))
		else stop("Unknown class for input")
	}
...
SeuratCellListQuery <- function(sro_or_sce, metaquery=c(), listquery=c(),default=T,getnames=F,negate=F){
	if (class(sro_or_sce) == "SingleCellExperiment"){ metadata = colData(sro_or_sce)
	}else{metadata = sro_or_sce@meta.data}
	if (!is.null(listquery)){
		if (length(listquery) != nrow(metadata)) {
			listquery <- !is.null(match(listquery, rownames(metadata)))
		}else{
			listquery[is.na(listquery)] <- F
			listquery <- listquery
		}
		...
		if (is.null(metaquery)) return(rep(default,nrow(metadata)))
		listquery <- rep(T,nrow(metadata))
	}
	if (class(metaquery) == "list"){
		dalist <- rep(T,nrow(metadata))
		for(i in names(metaquery)){
			if (!i %in% colnames(metadata)) stop(paste("Missing", i, "in seurat meta.data"))
			listquery = listquery & (metadata[[i]] %in% c(metaquery[[i]]))
		}
	}else{
	if (length(metaquery) == 1) {
		if (!metaquery %in% colnames(metadata)) stop(paste("Missing", metaquery, "in seurat meta.data"))
		dalist <- metadata[[metaquery]]
	}else{
		if (!metaquery[1] %in% colnames(metadata)) {
			if (substr(metaquery[1],1,1) %in% c(">", "<")){
				if (substr(metaquery[1],2,2) == "=") {
					if (substr(metaquery[1],1,1) == "<") dalist <- metadata[[substr(metaquery[1],3,10000)]] <= as.numeric(metaquery[2])
					else dalist <- metadata[[substr(metaquery[1],3,10000)]] >= as.numeric(metaquery[2])
				}else{
					if (substr(metaquery[1],1,1) == "<") dalist <- metadata[[substr(metaquery[1],2,10000)]] < as.numeric(metaquery[2])
					else dalist <- metadata[[substr(metaquery[1],2,10000)]] > as.numeric(metaquery[2])
				}
				dalist[is.na(dalist)] <- F
			}else{
				if (substr(metaquery[1],1,1) == "!"){
				...
				if (!metaquery[1] %in% colnames(metadata)) stop(paste("Missing", metaquery[1], "in seurat meta.data"))
				dalist <- !is.na(match(metadata[[metaquery[1]]] , metaquery[2:length(metaquery)]))
			}
		}else dalist <- !is.na(match(metadata[[metaquery[1]]] , metaquery[2:length(metaquery)]))
	}
        }
        if (negate) dalist <- !dalist
        if (getnames) return(rownames(metadata)[dalist & listquery]) else return(dalist & listquery)
}
...
DEcalcIntersection <- function(sro, meta.partition=c(), meta.positive=c(),  cell.positive =c(),meta.use=c(), cell.use=c(), use.meta.as.replicate= c(),nb.replicates = 0, cell.weight =c(), bypass.rownames=c(), do.gprofile=T, gene.use=c(), gene.metause=c(), enqueue.go=F, namask.pvalue.threshold=0, bypass.negclass=c(),do.fdr.correction=F, do.permutation.test=c("none","cell","sample"),nb.partition.permute=0, sampling.meta=c(),min.nbcell.threshold=10){
	...
	if (is.null(meta.partition)) lvl <- "All"
		...
		if ((class(metadata[[meta.partition]]) == "factor")||("factor" %in% class(metadata[[meta.partition]]))) lvl <- levels(metadata[[meta.partition]])
		else lvl <- unique(metadata[[meta.partition]])
	}else{
		lvl <- meta.partition[2:length(meta.partition)]
		cell.use <- cell.use & SeuratCellListQuery(sro, meta.partition)
		meta.partition <- meta.partition[1]
	...
	if (is.null(cell.weight)) cell.weight <- rep(1,length(cell.positive))
	...
	if (!is.null(sampling.meta)) cell.use <- cell.use & (runif(length(cell.use)) < metadata[[sampling.meta]]);

	print(paste("DE for ", sum(cell.use & cell.positive), "against", sum( cell.use & !cell.positive) ,"cells to perform"));


	if (nb.partition.permute != 0){
		permute.pval <- data.frame(row.names=bypass.rownames)
		print("Counting cell in celltypes:")	
		if (!is.null(use.meta.as.replicate)) dafactor <- as.factor(paste(metadata[[use.meta.as.replicate]],cell.use))
	...
	for(i in 1:length(lvl)){
		if (is.null(meta.partition)) flt <- cell.use
		else flt <- (metadata[[meta.partition]] == lvl[i]) & cell.use
		...
		if ((sum(flt & cell.positive) == 0)|(sum(flt & !cell.positive) == 0)) {
			pval[[ lvl[i] ]] <- rep(1, length(bypass.rownames))
			log2FC[[ lvl[i] ]] <- rep(0, length(bypass.rownames))
			meanExp[[ lvl[i] ]] <- rep(0, length(bypass.rownames))
			next
		}
	...
	#	out = tryCatch({DEcalc(sro, flt & cell.positive, flt & !cell.positive, nb.replicates, use.meta.as.replicate=use.meta.as.replicate,gene.use=gene.use)
		...
		if (nb.partition.permute != 0){
			...
			tmptmp <- match(lvl[i], levels(metadata[[meta.partition]]))
...
#	}
	...
	for(i in 1:ncol(pval)) {
		if (class(pval[[i]]) != 'numeric'){
		print(lvl)
		print(pval)
		print(colnames(pval))		
		pval[[i]] <- rep(1, length(bypass.rownames))
		#print(pval[[colnames(pval)[i]]])
		#print(as.numeric(pval[[i]]))
		#print(class(pval[[i]]))
		#pval[[i]] = as.numeric(pval[[i]])
		}
	}
	print("haha")
...
DoThatBalance <- function(flt, ispos, partition){
	count <- table(partition[flt], ispos[flt])
	print(count)
return(flt)}

WilcHyperIntersection <- function(sro, scp, sampling.prob=c(), meta.partition=c(), meta.positive=c(),  cell.positive =c(),meta.use=c(), cell.use=c(), meta.partition.to.combine=c(),do.partition.Rside=F, cell.weight=c(),bypass.rownames=c(),namask.pvalue.threshold=0,do.plotZscores=0,do.quartile.average=T, do.permutation.test=F, nb.threads=4,do.downsample=F,do.reorder.bysignif=F,nbpart=4, do.seqtoo=c(), fancy_cell_balance=F){
	library(InferN0)
	if ((class(sro) != "seurat")&(class(sro) != "Seurat")&(class(sro) != 'SingleCellExperiment') ) stop("Expects a seurat object!")
	if (class(scp) != "InferN0Scope") stop("Expects an InferN0 scope!")

	if (!is.null(do.seqtoo)){
		#if (!("userep" %in% do.seqtoo)) do.seqtoo$userep = c()
		#print(do.seqtoo)
		res <- DEcalcIntersection(sro, meta.partition=meta.partition, meta.positive = meta.positive,  cell.positive = cell.positive,meta.use=meta.use, cell.use=cell.use, use.meta.as.replicate= do.seqtoo$userep)
		#print(names(res))
	}else res = list()
	
	meta.data = SeuratRawData(sro, "M")

	cell.use <- SeuratCellListQuery(sro, meta.use , cell.use)
	if (fancy_cell_balance) cell.use <- DoThatBalance(cell.use, colData(sro), meta.partition) 

	if (is.null(meta.partition)) lvl <- "All"
	else if (length(meta.partition) == 1) {
		if ((class(meta.data[[meta.partition]]) == "factor")||("factor" %in% class(meta.data[[meta.partition]]))) lvl <- levels(meta.data[[meta.partition]])
		else lvl <- unique(meta.data[[meta.partition]])
	}else{
		lvl <- meta.partition[2:length(meta.partition)]
		cell.use <- cell.use & SeuratCellListQuery(sro, meta.partition)
		meta.partition <- meta.partition[1]
	...
	if (!is.null(meta.partition.to.combine)) cplvl <- unique(meta.data[[meta.partition.to.combine]])

	cell.positive <- SeuratCellListQuery(sro, meta.positive, cell.positive)
	if (fancy_cell_balance) cell.use <- DoThatBalance(cell.use, cell.positive, colData(sro)[[meta.partition]])

	#gene.use <- SeuratGeneListQuery(sro, gene.use, gene.metause)
	#if (is.null(gene.use))
	gene.use = rep(T, length(SeuratRawData(sro,"R")))
	if (is.null(cell.weight)) cell.weight <- rep(1,nrow(meta.data))
	if (is.null(bypass.rownames)) {bypass.rownames <- make.names(SeuratRawData(sro,"R")[gene.use],unique=T)}
	...
	if (!is.null(sampling.prob)) sample.succ <- (runif(nrow(meta.data)) < sampling.prob)
	else sample.succ <- rep(T, nrow(meta.data))

	for(i in 1:length(lvl)){
		if (is.null(meta.partition)) flt <- cell.use
		else flt <- (meta.data[[meta.partition]] == lvl[i]) & cell.use
		...
		if (is.null(meta.partition.to.combine)||(do.partition.Rside == F)){
			if (!is.null(meta.partition.to.combine)) {
				...
				out <- InferN0ZeroCorrectedWilcox2(scp, flt & cell.positive, flt & !cell.positive, list.partition = meta.data[[meta.partition.to.combine]]@.Data, total_count=size,do.plotZscores=do.plotZscores,do.quartile.average=do.quartile.average, nb.threads= nb.threads,do.downsample=do.downsample,nbpart=nbpart)
			...
			for( j in 1:length(cplvl)){
				print(paste("within ", cplvl[j], ", which is ", sum( (meta.data[[meta.partition.to.combine]] == cplvl[j]) &  flt & cell.positive), "against", sum( (meta.data[[meta.partition.to.combine]] == cplvl[j]) & flt & !cell.positive) ));
				nbcells = sum((meta.data[[meta.partition.to.combine]] == cplvl[j]) &  flt)
				if (nbcells != 0){ 
			out2 <- InferN0ZeroCorrectedWilcox2(scp, (meta.data[[meta.partition.to.combine]] == cplvl[j]) & flt & cell.positive , (meta.data[[meta.partition.to.combine]] == cplvl[j]) & flt & !cell.positive, total_count=size,do.plotZscores=do.plotZscores, nb.threads= nb.threads,nbpart=nbpart)
			if (!is.null(out2)){
...
return(c(list(wilcox.log10pval = log10pval, wilcox.zscore= Zscore, wilcox.log2FC= log2FC,wilcox.logitAuroc= LogitAuroc,  meanTPM=meanTPM, CoverageEnrichment= CoverageEnrichment, dropoutPosClass=dropoutPosClass,dropoutNegClass=dropoutNegClass, Weight=Weight),res))}
...
getExtremes <- function(datalist, maxlist.toreport= 100, plot.maxlist=25, value.name= c(), meta.names=c(), pval.grep.string= "pval", pval.threshold=0.05, filter.name.and.val=c(), filter.table=c(), do.fdr.correction=F,gprofile.args=list(hier_filtering="strong",ordered_query=T),gprofile.nbterms=100,plot.attribs=c(),selected.colnames=c(), extrafilter=c(),make.ggplots=F,funcfilt=c(), use.funky.threshold=c()){
...
#		print(dim(mflt))
	...
	if (length(selected.colnames) != 0){
		...
		if (!is.null(extrafilter)) {
			if (length(extrafilter) ==1) curf <- (datalist[[extrafilter]][,selected.colnames[i],drop=T] > 0.05) #use dropout threshold?
			else{
				curf <-         datalist[[extrafilter[1]]][,selected.colnames[i],drop=T] > extrafilter[2]
				curf <- curf|(datalist[[extrafilter[3]]][,selected.colnames[i],drop=T] > extrafilter[4])
			}
		...
		if (nrow(toorder) > 0) {
    			if (!"custom_bg" %in% names(gprofile.args)) {
				#thisgarg <- c(list(custom_bg=intersect(rownames(toorder), funcfilt)), gprofile.args)
				thisgarg <- c(list(custom_bg=rownames(toorder)), gprofile.args)
			}else thisgarg <- gprofile.args
			print(intersect(rownames(toorder), funcfilt))
...
# just changed nbterm max...
getExtremesGP2 <- function(datalist, orgk, maxlist.toreport= 100, plot.maxlist=25, value.name= c(), meta.names=c(), pval.grep.string= "pval", pval.threshold=0.05, filter.name.and.val=c(), filter.table=c(), do.fdr.correction=F,gprofile.args=list(ordered_query=T),gprofile.nbterms=100,plot.attribs=c(),selected.colnames=c(), extrafilter=c(),make.ggplots=F,funcfilt=c(), use.funky.threshold=c()){
...
if (nrow(toorder) > 0) {
	...
	if (!is.null(funcfilt)) query <- query[query %in% funcfilt]
			...
			outputH <- do.call(gost,c(list(query=query,organism = orgk,evcodes=T) ,thisgarg))
			#outputH <- outputH$result
			print(query)
			...
			#outputH <- parseGprofiler2(outputH$results, bg.size = length(thisgarg$custom_bg),query=query,plot.maxlist=plot.maxlist,plot.attribs = plotAttribs(plot.attribs,title=paste("High in", selected.colnames[i])),gprofile.nbterms=gprofile.nbterms,gene.annot=toorder[ord,],make.ggplots=make.ggplots)
			tmp <- outputH$result[,c("p_value","term_name","intersection_size","intersection")]
			
			print(dim(tmp))
			tmp[["celltype"]] <- selected.colnames[i]
			tmp[["tail"]] <- "upregulated"
			print(dim(tmp))
			if (is.null(dim(tmp))) print("nothiG")
			else if (is.null(fout)) fout <- tmp
			else fout <- rbind(fout,tmp)
		...
		if (locflt != 0) {
			...
			#fout[[gsub(" ", "_",paste("BGO", selected.colnames[i],sep="_"))]] <- outputH
		        tmp <- outputH$result[,c("p_value","term_name","intersection_size", "intersection")]
			tmp[["celltype"]] <- selected.colnames[i]
			tmp[["tail"]] <- "downregulated"
			print(dim(tmp))
			if (is.null(dim(tmp))) print("nothing")
			else if (is.null(fout)) fout <- tmp
			else fout <- rbind(fout,tmp)	
...
makeTableFromExtremes <- function(res, path ="res.csv"){


	fout = data.frame()


	fout[nrow(df) + 1,] = c(value1, value2, ...)
...
makeSeuratFromSingleCellExperiment <- function(sce, celllist =c(), assay.raw.data.name="counts", makeSparse=T) {
	...
	}else rawcounts <-assays(sce)[[assay.raw.data.name]][, celllist] 
	print
...
plotLabelsFromSCE <- function(sce, key,plot.attribs=c(), text = c(), color = c()){
	library(SingleCellExperiment)
#	plotLabels(reduced, key)[,1]
	if (is.null(text)) text <- colnames(sce)
	else if (is.factor(colData(sce)[[text]])) text <- as.character(colData(sce)[[text]])
	else text <- colData(sce)[[text]]
	if (!is.null(color)) colorval = colData(sce)[[color]]
	else colorval = c()
return(plotLabels(reducedDims(sce)[[key]][,1], reducedDims(sce)[[key]][,2], text, color = colorval,colorname= color ,plot.attribs));}


plotLabels <- function(xdata, ydata, names = c(), color = c(), alpha = c(), filter = c(), xlim=c(), ylim=c(), colorname = c(), plot.attribs=c(),label.size=4, na.color= "#AAAAAA"){
	...
	if (continuous_color){
		colpal <- colorRampPalette(c("#00FFFF", "#00B0FF","#0079FF","#0000E8", "#000074","#000000","#4B0000","#960000","#E10000","#FF8000","#FFD600"))(41)[1:41]
		p <- p + scale_color_gradientn(name=colorname,colours=colpal, na.value= na.color)
	}else{
...
runMiloPipeline <- function(sce, sample_str = "sample_names", cond_str = "Stage", cellflt = c(), prefix = ""){
	library(miloR);	library(SingleCellExperiment);	library(ggplot2); library(dplyr)
	if (is.null(cellflt)) {
		mlo <- Milo(sce)
		reducedDim(mlo, "UMAP") <- reducedDim(sce, "UMAP")
	}else mlo <- Milo(sce[,cellflt]) 
	
	colData(mlo)[[sample_str]] <- factor(colData(mlo)[[sample_str]]) # make factor and removes used categories
	colData(mlo)[[cond_str]] <- factor(colData(mlo)[[cond_str]])

	nbdim = dim(reducedDim(mlo, "PCA"))[2]
	print("make graph")
	mlo <- buildGraph(mlo,k=10,d=nbdim)
	mlo <- makeNhoods(mlo, prop=0.1,k=10,d=nbdim,refined=TRUE)

#	df <- data.frame(nh_size = colSums(nhoods(mlo)))
#	ggplot(data = df, aes(nh_size)) + geom_histogram(bins = 50) + xlab("Neighbourhood size")

	mlo <- countCells(mlo, meta.data = data.frame(colData(mlo)), sample=sample_str)
	print("Make design")
	traj_design <- data.frame(colData(mlo))[,c(sample_str, cond_str)]
	traj_design <- distinct(traj_design)
	print("compute Nhood")
	rownames(traj_design ) = traj_design[,1] 

	mlo <- calcNhoodDistance(mlo, d=nbdim)
	print("Nhood done!")
	da_results <- testNhoods(mlo, design = ~ Stage, design.df = traj_design)
	funt = t(mlo@nhoods)
	print(dim(funt))
	print("lets fill")
#	if (!is.null(cellflt)) {
#		for( tmp in paste(prefix,c("FDR", "logFC", "spFDR"),sep='_')){
#			if (!(tmp %in% names(colData(sce)))) colData(sce)[[tmp]] <- NA
#		}
#		damap <- which(cellflt)
#		# match(colnames(mlo), colnames(sce))
#		for(i in 1:ncol(funt)){
#			www <- which(funt[,i] != 0)
#			if (!is.null(www)){
#				dares <- Matrix::colSums(da_results[www,]) / length(www)
#				colData(sce)[[paste(prefix,"FDR",sep='_')]][damap[i]] <- dares[5]
#				colData(sce)[[paste(prefix,"logFC",sep='_')]][damap[i]] <- dares[1]
#				colData(sce)[[paste(prefix,"spFDR",sep='_')]][damap[i]] <- dares[7]
#			}
#		}
#	}else{
#
...
#			if (!is.null(www)){
#				dares <- Matrix::colSums(da_results[www,]) / length(www)
#				colData(sce)[[paste(prefix,"FDR",sep='_')]][i] <- dares[5]
#				colData(sce)[[paste(prefix,"logFC",sep='_')]][i] <- dares[1]
#				colData(sce)[[paste(prefix,"spFDR",sep='_')]][i] <- dares[7]
#	
#			}
#		}
#	}
	...
	map = match(colnames(mlo)[as.integer(colnames(mlo@nhoods))], colnames(sce))
	print(length(map))
	print(length(da_results$FDR))
	colData(sce)[[paste(prefix,"FDR",sep='_')]][map] <-   da_results$FDR
	colData(sce)[[paste(prefix,"logFC",sep='_')]][map] <- da_results$logFC
	colData(sce)[[paste(prefix,"spFDR",sep='_')]][map] <- da_results$SpatialFDR
	


return(list(mlo=mlo, res = da_results ,design = traj_design, sce = sce))
#return(list(mlo=mlo, res= da_results, design = traj_design))
}

doAllDE <-function(sce, scp, meta.partition, meta.pos, meta.use){
	metadata <- colData(sce)
	if ((class(metadata[[meta.partition]]) == "factor")||("factor" %in% class(metadata[[meta.partition]]))) lvl <- levels(metadata[[meta.partition]])
	else lvl <- unique(metadata[[meta.partition]])
	outputlist <- list()
	outputlist$rawQDE = DEcalcIntersection(sce,  meta.partition=meta.partition, meta.pos = meta.pos, meta.use = meta.use, useuse.meta.as.replicate="sample_names")
	outputlist$rawWDE = WilcHyperIntersection(sce, scp,  meta.partition=meta.partition, meta.pos = meta.pos, meta.use = meta.use)
	outputlist$signifQDE = getExtremes(ez4, value.name="deseq.log2FC", meta.names= c("deseq.basemean", "dropoutPosClass", "dropoutNegClass" ), do.fdr.correction=T)
	outputlist$signifWDE = getExtremes(eww, value.name="wilcox.logitAuroc", meta.names= c("meanTPM", "dropoutPosClass", "dropoutNegClass" ), do.fdr.correction=T)
	for(ct in lvl){
